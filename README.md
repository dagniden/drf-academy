...
- [ ] Подключить `celery-beat` для периодических задач
- [ ] Вынести настройки Redis в переменные окружения
- [ ] Реализовать асинхронную рассылку писем при обновлении курса
- [ ] Вызывать задачу отправки письма из контроллера обновления курса
- [ ] Отправлять уведомления только подписанным на конкретный курс пользователям
- [ ] *Добавить проверку: уведомление отправляется только если курс не обновлялся более 4 часов*
- [ ] Реализовать периодическую задачу блокировки неактивных пользователей
- [ ] Проверять поле `last_login`
- [ ] Устанавливать `is_active=False`, если пользователь не заходил более месяца
- [ ] Настроить расписание задачи в `celery-beat`
- [ ] Проверить, что `timezone` приложения и Celery совпадают
